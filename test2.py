import os
import regex as re # Import regex here
from collections import Counter
from typing import  BinaryIO

#from minbpe.bpe import train_bpe_tokenizer  #file location
# Removed the problematic import: from pzHWPJT82vBM import RegexTokenizer # Import the RegexTokenizer class

# Define the RegexTokenizer class directly in this cell
# based on the content from cell pzHWPJT82vBM
"""
...
import unicodedata # Also need unicodedata for the helper functions
...
def get_stats(ids, counts=None):
    """
    writing a function that find the most common pair
...
# the rest of the original code in cell ImOLQIYDtYor
...
## this cell reads a large file, splits it into manageable chunks based on a special token,
##and then counts the occurrences of each word within those chunks, aggregating the counts into a single global_word_freqs counter.
##This aggregated counter is then used in the next step to train the BPE tokenizer.
...
def train_bpe_tokenizer(input_path: str, vocab_size: int, special_tokens: list[str]):
    ...
    num_chunks = 8 # This can be adjusted based on file size and memory

    # Global frequency collector
    ...
    # Train the tokenizer using the RegexTokenizer class
    tokenizer = RegexTokenizer()
    # We need to pass the full text to the train method, not just the word frequencies.
    # Read the entire text file for training
